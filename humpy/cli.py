...
from humpy.memory.store import loadIndexEntry,sessionTurnCount
from humpy.config import loadAgentCfg
from humpy.utils import fmtTs

RESERVED_BOT_INPUT={'new','others'}

def primaryBot(allBot,defaultBot,n):
    primary=[]
    if defaultBot in allBot and not Bot.isReserved(defaultBot):
...
def pickFromOthers(primary,allBot,limit):
    rest=[b for b in allBot if b not in primary and not Bot.isReserved(b)][:limit]
    if not rest:
        print('no other bots')
        return None
...
def pickBotInteractive(cfg):
    allBot=Bot.listName()
    primary=primaryBot(allBot,cfg['defaultBot'],cfg['primaryBotShow'])
    if primary:
        shown=', '.join(primary)
    else:
        shown='(none yet)'
    ...
    while True:
        ...
        if pl=='new':
            ...
            continue
        if pl=='others':
            bot=pickFromOthers(primary,allBot,cfg['othersBotLimit'])
            if bot:
                return bot
            continue
        bot=Bot.adopt(pick)
...
def pickSession(bot,argNew,argResume,menuLimit):
    ...
    if argNew:
        return None,False
    entry=loadIndexEntry(bot.indexFile,limit=menuLimit)
    entry=list(reversed(entry))
    print('0. new session')
    ...
    if pick.isdigit():
        ...
        if n==0:
            return None,False
        if 1<=n<=len(entry):
            return entry[n-1]['sessionId'],True
...
def cmdChat(args):
    ...
    lstLimit=cfg.get('sessionsListLimit',cfg['sessionMenuLimit'])
    ...
    while True:
        ...
        if isCommand(userText):
            shouldExit,sess=dispatch(userText,sess,pickId=args.model_id,lstLimit=lstLimit)
            if shouldExit:
                break
            continue
        ...
        out=sess.turn(userText,maxToken=maxTok)
